"""Import-completeness tests for Agent.
...
def _import_check(module_expr: str, workspace: str) -> subprocess.CompletedProcess:
    ...
    PYTHONPATH is extended with the project root so that ``agents.Agent``
...
class TestAgentImports(unittest.TestCase):
    """Verify that every Agent sub-module imports without errors."""
    ...
    def test_constants(self):
        self._assertImportable("agents.Agent.constants")

    def test_workspace(self):
        self._assertImportable("agents.Agent.workspace")

    def test_guide(self):
        self._assertImportable("agents.Agent.guide")

    def test_skill(self):
        self._assertImportable("agents.Agent.skill")

    def test_orchestrator(self):
        self._assertImportable("agents.Agent.agents.orchestrator.agent")

    def test_thinking_agent(self):
        self._assertImportable("agents.Agent.agents.thinking_agent")

    def test_execution_agent(self):
        self._assertImportable("agents.Agent.agents.execution_agent")

    def test_tester_agent(self):
        self._assertImportable("agents.Agent.agents.tester_agent")

    # ── Top-level package ─────────────────────────────────────────────────────

    def test_top_level_exports_app(self):
        """agents.Agent must be importable and export `app`."""
        result = _import_check(
            "agents.Agent; from agents.Agent import app",
            self.workspace,
        )
        self.assertEqual(
            result.returncode, 0,
