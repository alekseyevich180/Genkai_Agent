...
Examples:
    agent web
    agent web --reload-agents --port 8080
    agent api-server
    agent run -p "Build a silicon FCC structure"
    agent run -f prompt.txt --output-format json -o result.json
    MATCLAW_WORKSPACE=/data/ws agent run -p "hello"
"""
...
_CONFIG_PATH = Path("~/.agent/config.yaml").expanduser()


def _resolve_project_root() -> tuple[Path, bool]:
    """Resolve the Agent project root.
    Resolution order (first match wins):
      1. ``AGENT`` environment variable
      2. ``project_root`` in ``~/.agent/config.yaml``
      3. Fallback: derive from ``__file__`` (works for editable / source installs)
    Returns (path, explicitly_configured).
    """
    # 1. Environment variable
    env_val = os.environ.get("AGENT")
    if env_val:
...
async def run_agent_async(
    ...
    # agents/ lives at PROJECT_ROOT, which may not be on sys.path when agent
    ...
    from agents.Agent.agent import app

    # Use a persistent SQLite session service so sessions written by `agent run`
    # appear in the web frontend (which reads agents/Agent/.adk/session.db).
    _adk_db = AGENTS_DIR / "Agent" / ".adk" / "session.db"
    ...
    return {
        "answer": answer,
        "model_name": os.environ.get("LLM_MODEL", "agent"),
...
# ---------------------------------------------------------------------------
# `agent knowledge` subcommand group
# ---------------------------------------------------------------------------
...
def knowledge_query(text, top_k, depth, workspace):
    ...
    from agents.Agent.knowledge.query import query_knowledge_graph
...
def knowledge_search_skills(text, top_k, workspace):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.knowledge.query import search_skills
...
def knowledge_related_skills(start_node, top_k, depth, workspace):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.knowledge.query import get_related_skills
...
def knowledge_stats(workspace):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.knowledge.kdg_memory import iter_memory
    from agents.Agent.knowledge.query import _get_kg
    graph = _get_kg()
    stats = graph.stats()
...
def knowledge_seed(workspace):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.skill import seed_skills_to_graph
...
def knowledge_migrate(workspace, memory_md):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.constants import KNOW_DO_GRAPH_DB
    from agents.Agent.knowledge.kdg_memory import iter_memory
    from agents.Agent.knowledge.migrate import migrate_memory_md
    from agents.Agent.knowledge.query import _get_kg, get_migration_result
...
def knowledge_distill(min_evidence, stale_days, workspace):
    ...
    _setup_workspace(workspace)
    _ensure_path(PROJECT_ROOT)
    from agents.Agent.knowledge.synthesizer import run_knowledge_synthesizer
