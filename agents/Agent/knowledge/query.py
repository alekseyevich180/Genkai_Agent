...
def _configure_auto_review(graph: KnowDoGraph) -> None:
    ...
    enabled = os.environ.get("AGENT_AUTO_REVIEW", "1").strip().lower()
    ...
    batch_size = int(os.environ.get("AGENT_REVIEW_BATCH_SIZE", "5"))
    graph.auto_review(
        threshold=threshold,
...
def get_related_skills(start_node: str, top_k: int = 5, depth: int = 2) -> str:
    ...
    try:
        ...
        if start is None:
            matches = graph.search(start_node, tags=["agent-skill"], limit=1)
