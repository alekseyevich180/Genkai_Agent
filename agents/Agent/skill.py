"""Markdown-backed workflow skills and search utilities for Agent.
...
class AgentSkillToolset(skill_toolset.SkillToolset):
...
ALL_SKILLS_TOOLSET = AgentSkillToolset(ALL_SKILLS)
...
def seed_skills_to_graph() -> dict:
    ...
    for skill in ALL_SKILLS:
        node, created = upsert_entry(
            ...
            metadata=EntryMetadata(
                ...
                custom={"managed_by": "agent", "kind": "skill"},
            ),
        )
    ...
    for guide in ALL_GUIDES:
        _, created = upsert_entry(
            ...
            tags=["agent-skill", "agent-guide", "managed"],
            metadata=EntryMetadata(
                source_provenance="guide",
                refinement_status=RefinementStatus.validated,
                verification_status=VerificationStatus.peer_reviewed,
                custom={"managed_by": "agent", "kind": "guide"},
            ),
        )
        seeded += int(created)
